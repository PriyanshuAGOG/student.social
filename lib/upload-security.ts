...
export function scanUploadMeta(file: { name?: string; type?: string; size?: number }, options: { maxBytes?: number } = {}): UploadScanResult {
  const name = (file.name || '').toLowerCase()
  const type = (file.type || '').toLowerCase()
  const size = file.size || 0

  if (size > (options.maxBytes || MAX_FILE_SIZE_BYTES)) return { ok: false, reason: 'file too large' }
