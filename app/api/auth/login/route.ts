...
function respondAuthError(status: number, code: string, message: string, details?: Record<string, unknown>) {
  return authErrorResponse({ status, code, message, details })
}
...
export async function POST(req: Request) {
  try {
    ...
    if (!envCheck.ok || !endpoint || !project || !cookieSecret) return respondAuthError(500, 'AUTH_ENV_MISSING', 'Authentication server is misconfigured.', { missing: envCheck.ok ? [] : envCheck.missing })
    ...
    if (!appwriteResp.ok) {
      ...
      return respondAuthError(appwriteResp.status || 401, mapped.code, mapped.message, { appwriteType: appwriteData?.type || null })
    }
    ...
    return respondAuthError(400, 'LOGIN_REQUEST_INVALID', error?.message || 'Login failed')
  }
}
